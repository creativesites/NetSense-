...
class RecoveryOutcomeTracker(private val dao: RecoveryOutcomeDao) {
    ...
    companion object {
        ...
        /** How long to wait for real recovery before giving up and marking FAILED. Was 90s, then
         *  45s - both made the Home healing screen feel stuck for an unexplained wait. Most of
         *  these actions (radio re-registration after an airplane-mode toggle, DNS
         *  re-resolution) settle well within 20s in practice, so there's no honest reason to
         *  keep the user waiting longer before offering a retry; a real attempt that's still
         *  actively resolving stays PENDING past this on its own (see the "still pending" branch
         *  below) rather than being force-declared FAILED mid-flight. */
        const val RECOVERY_TIMEOUT_MS = 20_000L
